...
def draw_ent(prediction, save_root, name):
    ...
    if not os.path.exists(os.path.join(save_root, 'boun')):
        os.makedirs(os.path.join(save_root, 'boun'))
    smooth = 1e-8
    cup = prediction[0]
    disc = prediction[1]
    boun = prediction[2]
    ...
    cv2.imwrite(os.path.join(save_root, 'boun', name.split('.')[0]) + '.png', boun)
...
def draw_mask(prediction, save_root, name):
   ...
   '''
    ...
    prediction = postprocessing(prediction, 0.75, 'G')
    cup = prediction[0]
    disc = prediction[1]
