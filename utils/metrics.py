...
def calculate_bd(mask_numpy, gt_numpy):
    """
...
def compute_surface_distances(mask_gt,
  ...
  mask_gt = np.asarray(mask_gt, dtype=np.bool)
  mask_pred = np.asarray(mask_pred, dtype=np.bool)
...
def compute_average_surface_distance(pred, label):
    # Only consider non-background classes (class 3 is the background)
    classes = [0, 1]
    ...
    for c in classes:
        # Get binary masks for the predicted and label volumes for the current class
        pred_mask = (pred == c)
        label_mask = (label == c)

        # Compute the distance transform of the predicted and label masks
        pred_dt = distance_transform_edt(pred_mask)
        label_dt = distance_transform_edt(label_mask)

        # Compute the surface of the predicted and label masks
        pred_surface = pred_dt == 1
        label_surface = label_dt == 1

        # Compute the distance between the surfaces of the predicted and label masks
        asd = np.logical_xor(pred_surface, label_surface).sum() * 1.0
        asd /= (label_surface.sum() + pred_surface.sum())

        # Add the average surface distance for the current class to the sum
        asd_sum += asd
        num_voxels += 1
        #print(asd)

    # Compute the mean average surface distance across all classes
    mean_asd = asd_sum / num_voxels
