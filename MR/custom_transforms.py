...
def to_trilabel(pre_mask, boundary):
    mask = np.zeros((pre_mask.shape[0], pre_mask.shape[1], 3))
    mask[..., 0] = pre_mask[..., 0]
    mask[..., 1] = pre_mask[..., 1]
    mask[..., 2] = np.squeeze(boundary)
    return mask
...
class Normalize_tf(object):
    ...
    def __call__(self, sample):
        ...
        _mask[__mask > 150] = 255

        __mask[_mask == 0] = 0
        __mask[_mask == 255] = 2

        ___mask = to_multilabel(__mask)
        #soft方法提取骨架的代码
        #mask1 = np.array(___mask[..., 0]).astype(np.float32)
        #mask1 = torch.from_numpy(mask1).float()
        #mask1 = mask1.unsqueeze(0)
        #mask1 = mask1.unsqueeze(0)
        #boundary = soft_skeleton.soft_skel(mask1, 10)
        #boundary = torch.nn.functional.interpolate(boundary, size=(mask1.size()[2], mask1.size()[3]),
        #                                            mode="bilinear")
        #boundary = boundary.numpy().astype(np.uint8)

        mask1 = np.array(___mask[..., 0]).astype(np.bool)
        boundary = skeletonize(mask1)
        boundary = boundary.astype(np.uint8)

        mask = to_trilabel(___mask, boundary)
        return {'image': img,
                'map': mask,
                'img_name': name
               }
