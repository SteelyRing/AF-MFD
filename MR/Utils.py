...
def draw_ent(prediction, save_root, name):
    ...
    if not os.path.exists(os.path.join(save_root, 'skel')):
        os.makedirs(os.path.join(save_root, 'skel'))
    ...
    skell = 255*skell
    cv2.imwrite(os.path.join(save_root, 'skel', name.split('.')[0]) + '.png', skell)
...
def draw_mask(prediction, save_root, name):
   ...
   '''
    ...
    if not os.path.exists(os.path.join(save_root, 'cup')):
        os.makedirs(os.path.join(save_root, 'cup'))
    if not os.path.exists(os.path.join(save_root, 'total')):
            os.makedirs(os.path.join(save_root, 'total'))

    prediction = postprocessing(prediction, 0.4, 'G')
    cup = prediction[0]
    disc = prediction[1]

    total = 255*(0.5*cup+0.5*disc)
    cv2.imwrite(os.path.join(save_root, 'total', name.split('.')[0]) + '.png', total)
...
def postprocessing(prediction, threshold=0.4, dataset='G'):
    if dataset[0] == 'D':
        prediction_copy = np.copy(prediction)
        disc_mask = prediction[1]
        cup_mask = prediction[0]
        disc_mask = (disc_mask > threshold)  # return binary mask
        ...
        disc_mask = prediction[1]
        cup_mask = prediction[0]
        new = (cup_mask > disc_mask)
        new = new.astype(np.uint8)
        prediction_copy[0] = new
        prediction_copy[1] = new
        return prediction_copy
...
def save_per_img(patch_image, data_save_path, img_name, prob_map, mask_path=None, ext="bmp"):
    ...
    new = (cup_map > disc_map)

    #disc_mask = (disc_map > 0.75) # return binary mask
    #cup_mask = (cup_map > 0.75)
    disc_mask = new.astype(np.uint8)
    cup_mask = new.astype(np.uint8)
    ...
    '''for n, contour in enumerate(contours_disc):
        ...
        patch_image[(contour[:, 0] - 1.0).astype(int), (contour[:, 1] - 1.0).astype(int), :] = [0, 0, 255]
        patch_image[(contour[:, 0]).astype(int), (contour[:, 1] - 1.0).astype(int), :] = [0, 0, 255]'''
    a = np.size(disc_mask, 0)
    b = np.size(disc_mask, 1)
    for i in range(a):
        for j in range(b):
            if disc_mask[i, j] == 1:
                patch_image[i, j, :] = [0, 0, 255]


    #patch_image[disc_mask] = [0, 0, 255]
